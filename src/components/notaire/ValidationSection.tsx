  ...
  return (
    <div className="space-y-4">
            ...
            <div key={docItem.type} className="rounded-xl overflow-hidden"
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 p-3">
                <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                  <FileText className="w-4 h-4 shrink-0 hidden sm:block" style={{ color: HColors.gold }} />
      ...
      {ready && !property.verified_notaire && (
        <div className="mt-4 sm:mt-6 p-3 sm:p-4 rounded-xl sm:rounded-2xl text-center space-y-2 sm:space-y-3"
          style={{ background: 'linear-gradient(135deg, #0D1F12, #1A2E1F)', border: `1px solid ${HAlpha.gold30}` }}>
          <BadgeCheck className="w-6 h-6 sm:w-8 sm:h-8 mx-auto" style={{ color: HColors.gold }} />
          <div>
            <p className="text-xs sm:text-sm font-bold text-cream" style={{ color: HColors.cream }}>Dossier complet & vérifié</p>
            <p className="text-[10px] text-cream/60">Tous les documents requis sont validés.</p>
          </div>
          <button onClick={() => handleCertify(property)} disabled={!!certifyingId}
            className="w-full py-2.5 sm:py-3 rounded-xl font-black uppercase tracking-widest text-[10px] sm:text-xs transition-all hover:scale-[1.02] disabled:opacity-50"
