...
import { 
  Users, Home, MapPin, Calendar, Building2, Eye, 
...
export const PropertyStatusBadge: React.FC<{ status: string }> = ({ status }) => {
...
export const UsersSection: React.FC<UsersSectionProps> = ({ 
  ...
  <div className="animate-in fade-in duration-500">
    ...
    <div className="rounded-2xl overflow-hidden shadow-sm" style={{ background: HColors.white, border: `1px solid ${HAlpha.gold15}` }}>
      <div className="flex items-center gap-3 px-5 py-3.5 flex-wrap" style={{ borderBottom: `1px solid ${HAlpha.gold10}`, background: 'rgba(249,243,232,0.5)' }}>
        <select value={filterRole} onChange={e => setFilterRole(e.target.value)} className="px-3 py-2 rounded-xl text-xs outline-none" style={{ background: HColors.white, border: `1px solid ${HAlpha.gold20}`, color: HColors.darkBrown }}>
        ...
        <span className="ml-auto text-xs" style={{ color: HColors.brown }}>{users.length} utilisateur(s)</span>
      </div>
              ...
              <tr key={user.id} style={{ background: i % 2 === 0 ? HColors.white : 'rgba(249,243,232,0.4)' }}>
                <td className="px-5 py-3.5">
                  <div className="flex items-center gap-2.5">
                    <div className="w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm" style={{ background: HAlpha.gold10, color: HColors.gold, border: `1px solid ${HAlpha.gold25}` }}>
                      {user.full_name?.charAt(0).toUpperCase()}
                    </div>
                    <div><div className="text-sm font-semibold" style={{ color: HColors.darkBrown }}>{user.full_name}</div><div className="text-xs" style={{ color: HColors.brown }}>{user.email}</div></div>
                  </div>
                </td>
    ...
    <div className="animate-in fade-in duration-500">
      ...
      <div className="mb-8 p-6 rounded-2xl shadow-sm border border-dashed flex flex-col md:flex-row md:items-center gap-6" style={{ background: HAlpha.orange08, borderColor: HAlpha.orange30 }}>
        ...
        <div className="flex gap-2 w-full sm:min-w-[300px]">
          <input type="text" value={tokenInput} onChange={e => setTokenInput(e.target.value.toUpperCase())} placeholder="JETON (ex: HC-XXXXXX)" className="flex-1 px-4 py-2.5 rounded-xl font-mono font-bold tracking-widest outline-none border border-orange-200" />
          <button onClick={handleApplyToken} disabled={consuming || !tokenInput} className="px-6 py-2.5 rounded-xl font-bold transition-all hover:opacity-90 disabled:opacity-50 flex items-center gap-2" style={{ background: `linear-gradient(135deg,${HColors.orangeCI},${HColors.gold})`, color: '#FFF' }}>
