    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500">Où se situe votre bien exactement ?</p>
        </div>
      </div>

      <div className="grid md:grid-cols-2 gap-4">
        <div>
          <label className="block mb-1.5" style={S.label}>District</label>
          <select name="district" value={formData.district} onChange={onChange} className={inputCls} style={S.input}>
            <option value="">Sélectionner</option>
            {ALL_DISTRICTS.map(d => <option key={d} value={d}>{d}</option>)}
          </select>
        </div>
        <div>
          <label className="block mb-1.5" style={S.label}>Région</label>
          <select name="region" value={formData.region} onChange={onChange} disabled={!formData.district} className={inputCls} style={S.input}>
            <option value="">Sélectionner</option>
            {regions.map(r => <option key={r} value={r}>{r}</option>)}
          </select>
        </div>
        <div>
          <label className="block mb-1.5" style={S.label}>Département</label>
          <select name="departement" value={formData.departement} onChange={onChange} disabled={!formData.region} className={inputCls} style={S.input}>
            <option value="">Sélectionner</option>
            {departements.map(d => <option key={d} value={d}>{d}</option>)}
          </select>
        </div>
        <div>
          <label className="block mb-1.5" style={S.label}>Ville *</label>
          <select name="city" value={formData.city} onChange={onChange} disabled={!formData.departement} className={inputCls} style={S.input}>
            <option value="">Sélectionner</option>
            {villes.map(v => <option key={v} value={v}>{v}</option>)}
          </select>
        </div>
        <div>
          <label className="block mb-1.5" style={S.label}>Commune</label>
          <select name="commune" value={formData.commune} onChange={onChange} disabled={!formData.city} className={inputCls} style={S.input}>
            <option value="">Sélectionner</option>
            {communes.map(c => <option key={c} value={c}>{c}</option>)}
          </select>
        </div>
        <div>
          <label className="block mb-1.5" style={S.label}>Quartier</label>
          <select name="quartier" value={formData.quartier} onChange={onChange} disabled={!formData.city} className={inputCls} style={S.input}>
            <option value="">Sélectionner</option>
      ...
      <div>
        <label className="block mb-1.5" style={S.label}>Adresse précise (optionnel)</label>
        <input type="text" name="address" value={formData.address} onChange={onChange} className={inputCls} style={S.input} placeholder="Ex: Rue du Commerce, Immeuble Horizon" />
      </div>
